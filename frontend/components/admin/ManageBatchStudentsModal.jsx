...
const ManageBatchStudentsModal = ({ isOpen, onClose, batch }) => {
    ...
    const { studentsInBatch, studentsNotInBatch } = useMemo(() => {
        if (!batch) return { studentsInBatch: [], studentsNotInBatch: [] };
        const inBatch = students.filter(s => s.batch === batch.id);
        const notInBatch = students.filter(s => s.role === 'STUDENT' && s.batch !== batch.id);
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Manage Students in ${batch.name}`} size="lg">
